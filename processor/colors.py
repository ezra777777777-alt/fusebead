# Perler 60-color palette — (code, name, r, g, b)
PERLER_COLORS = [
...
def redmean_distance(c1, c2):
    """Weighted red-mean color distance (better perceptual match than Euclidean)."""
    r1, g1, b1 = c1
    r2, g2, b2 = c2
    r_mean = (r1 + r2) / 2.0
    dr = r1 - r2
    dg = g1 - g2
    db = b1 - b2
    return ((2 + r_mean / 256) * dr ** 2 + 4 * dg ** 2 + (2 + (255 - r_mean) / 256) * db ** 2) ** 0.5


def find_closest_color(rgb, palette):
    """Return the closest palette color tuple (code, name, r, g, b) for the given RGB."""
    best = palette[0]
    best_dist = float("inf")
    for c in palette:
        d = redmean_distance(rgb, (c[2], c[3], c[4]))
        if d < best_dist:
            best_dist = d
            best = c
    return best
